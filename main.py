...
async def run_attack(update, target, port, duration):
    ...
    try:
        ...
        current_attack_process = process  # Store the process globally

        stdout, stderr = await process.communicate()
        current_attack_process = None  # Clear the process when done
...
async def pause(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        ...
        await update.message.reply_text("⏸️ Bot is now paused. Commands will not be processed.")
    else:
        await update.message.reply_text("❌ You are not authorized to use this command!")
...
async def resume(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        ...
        await update.message.reply_text("▶️ Bot is now active. Commands will be processed.")
    else:
        await update.message.reply_text("❌ You are not authorized to use this command!")


async def stop_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handler for the /stop_attack command."""
    global current_attack_process
    user_id = update.effective_user.id

    if user_id == ADMIN_ID:
        if current_attack_process and current_attack_process.returncode is None:
            current_attack_process.terminate()  # Stop the process
            await update.message.reply_text("⏹️ The ongoing attack has been stopped.")
